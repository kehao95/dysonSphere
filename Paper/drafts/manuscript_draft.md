...
The Dyson Swarm concept is often treated as the most plausible route toward stellar-scale energy collection, yet its conventional Keplerian formulation carries not only a collision-management problem but a deeper topology-and-growth problem. Large numbers of collectors placed at comparable radii inevitably occupy orbital planes that intersect at nodal points, so increasing swarm richness increases the network of geometric crossing corridors that must be phase managed. Symmetric phase-separated megaconstellation strategies can redistribute conjunction timing, but they do not geometrically remove the underlying intersections and are naturally defined for fixed constellation parameters rather than open-ended, incrementally growing infrastructure. At the opposite extreme, fully radiatively supported Dyson-bubble or statite concepts avoid such intersections altogether but require extreme areal-density thresholds approaching the $\beta \geq 1$ limit.
...
We do not claim a complete Dyson-engineering realization, nor a new foundation for solar-sail orbit theory. Instead, we make three narrower contributions. First, at the architectural level, we propose a Dyson support continuum: a parameterized view in which swarm-like and bubble-like configurations occupy different regions of the same support spectrum rather than forming a purely discrete taxonomy. Second, within that continuum, we identify MDDS as a low-$\beta$ micro-displaced regime that offers a candidate alternative architecture language to the conventional Keplerian Dyson swarm by shifting the organizing problem from managing an expanding nodal-intersection network to arranging layered, synchronizable latitude bands. Third, at the idealized-physics level, we show that the Sun-Earth environment contains a non-empty low-angle operating window in which modest angular displacement already yields large spatial stratification, while entry-level characteristic angles such as $\theta_\oplus$ and $0.1^\circ$ provide useful areal-density scale comparisons against present lightweight spacecraft systems. Representative examples at $0.1^\circ$, $0.5^\circ$, and $1^\circ$ show that the ideal-model low-latitude support window remains non-empty well below the full statite/bubble limit, although the allowable mass window narrows rapidly with latitude and becomes materially more demanding by $0.5^\circ$ and $1^\circ$, tightening much further again by $2^\circ$. The result is an analytic architectural framework, together with an identification of an idealized low-latitude regime worth further study.
...
### 1.7 Related work and novelty posture
...
The main result of this paper is best understood as a two-level reframing. At the architectural level, MDDS is not presented here as a completed Dyson-engineering solution, but as an alternative architecture language to the conventional Keplerian Dyson swarm, one that replaces intersecting swarm geometry with radiation-assisted latitude bands and thereby motivates a lower orbital-topology burden. More broadly, the paper argues that Dyson architectures are more coherently understood as a continuous support-and-stratification spectrum than as a purely discrete shell/swarm/bubble taxonomy. At the idealized-physics level, its significance is that it identifies a usefully screenable low-angle regime in which modest radiation-pressure support can produce large off-plane stratification without crossing into the payload-hostile bubble limit.
