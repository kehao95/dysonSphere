...
The Dyson Swarm concept is often treated as the most plausible route toward stellar-scale energy collection, yet its conventional Keplerian formulation contains a fundamental geometric difficulty: large numbers of collectors placed at comparable radii inevitably occupy orbital planes that intersect at nodal points. This creates a long-horizon collision-management problem that becomes more severe as swarm density increases. In contrast, fully radiatively supported Dyson-bubble or statite concepts avoid such intersections but require extreme areal-density thresholds approaching the $\beta \geq 1$ limit.

Building on established solar-sail displaced non-Keplerian orbit (DNKO) theory rather than proposing a fundamentally new orbit family, we argue that Dyson architectures are more usefully understood as a continuous support-and-stratification spectrum bridging the planar Keplerian swarm limit and the fully radiatively supported bubble/statite limit. Within that spectrum, we reinterpret the low-latitude heliocentric displaced-orbit regime as an analytic architecture framework for layered Dyson-swarm design. In this Micro-Displaced Dyson Swarm (MDDS) framing, collectors remain primarily orbital, but solar-radiation pressure is used to maintain small out-of-plane displacements and thereby create stratified latitude bands. For an ideal specular sail, this yields a closed-form low-$\beta$ support curve,

$$
\beta_{\min}(\phi)=\frac{3\sqrt{3}}{2}\sin\phi,
$$

and its equivalent system areal-density limit,

$$
\sigma_{\max}(\phi)=\frac{2\sigma^*}{3\sqrt{3}\sin\phi}.
$$

This converts the architecture question into a simple feasibility criterion: a given system is supportable at latitude $\phi$ if $\sigma_{\text{sys}} < \sigma_{\max}(\phi)$. The same framework naturally yields two design directions: a payload-optimized branch, obtained by minimizing $\beta$ at fixed $\phi$, and a synchronization-constrained branch, obtained by imposing an external orbital-period condition such as Earth-synchronous heliocentric motion. The latter is treated here as an operationally meaningful variant of known DNKO design space, not as a newly discovered orbit family.

We do not claim a complete Dyson-engineering realization, nor a new foundation for solar-sail orbit theory. Instead, we make three narrower contributions. First, at the architectural level, we propose a Dyson support continuum: a parameterized view in which swarm-like and bubble-like configurations occupy different regions of the same support spectrum rather than forming a purely discrete taxonomy. Second, within that continuum, we identify MDDS as a low-$\beta$ micro-displaced regime that offers a progressive alternative to the conventional Keplerian Dyson swarm by replacing intersecting swarm geometry with radiation-assisted latitude bands and thereby motivating a reduced orbital-topology burden. Third, at the physical-feasibility level, we show that the Sun-Earth environment contains a non-empty low-angle operating window in which modest angular displacement already yields large spatial stratification, while entry-level characteristic angles such as $\theta_\oplus$ and $0.1^\circ$ already correspond to areal-density thresholds comparable to present lightweight spacecraft systems. Representative examples at $0.1^\circ$, $0.5^\circ$, and $1^\circ$ demonstrate that substantial off-plane separation is achievable while remaining far below the full statite/bubble limit, although the allowable mass window narrows rapidly with latitude and becomes materially more demanding by $0.5^\circ$ and $1^\circ$, tightening much further again by $2^\circ$. The result is a theory-grounded architectural framework for stratified Dyson-swarm design, together with low-latitude illustrative feasibility slices.
...
- **Figure 3.** Latitude support curves $\beta_{\min}(\phi)$ and $\sigma_{\max}(\phi)$.
- **Figure 4.** Low-latitude illustrative window at $0.1^\circ$, $0.5^\circ$, and $1^\circ$.
- **Figure 5.** Earth-synchronous radius correction along the payload-optimized branch.

## 1. Introduction

### 1.1 Dyson swarms and the orbital-management problem
...
In a conventional Keplerian swarm, every orbital plane must pass through the central mass. As a result, non-coplanar orbits at comparable radii generically intersect at nodes. For sparse systems this may be tolerable; for dense systems it becomes a structural limitation rather than an operational detail. Phase management, ring nesting, and other organizational strategies can postpone the problem, but each introduces its own instability, shading, or thermal penalties. The result is a persistent gap between the appealing energy logic of the Dyson-swarm idea and the orbital logic needed to organize it at scale. Modern Dyson-swarm discussions have generally treated this at a qualitative level, often by invoking radial nesting or broad collector distributions, but without a compact architecture criterion for how to obtain topological separation at fixed stellar distance (Wright et al., 2015; McInnes, 2025, 2026).
...
### 1.2 The gap between Keplerian swarms and radiative-support concepts
...
This suggests that the most interesting design region may lie between these two familiar limits. A useful architecture would not need to fully levitate collectors, but it would need to displace them enough to prevent the topological crowding that characterizes a purely Keplerian swarm. The key question is whether such an intermediate regime exists in a mathematically clean and physically meaningful form.

### 1.3 Why a continuum view matters
...
This is also what makes the framework specifically valuable for Dyson theory rather than merely for sailcraft analysis. A stellar collector population is not judged only by the existence of one admissible orbit, but by whether there exists a scalable region of architecture space in which large numbers of collectors can be organized, expanded, and compared. The continuum claim is therefore a way of turning Dyson structures from a taxonomy of end states into a parameterized design landscape.

### 1.4 MDDS as a low-beta intermediate regime
...
The present paper sits between those two literatures. Its main novelty is not the introduction of a fundamentally new solar-sail orbit family. Nor does it claim that Earth-synchronous displaced orbits, period-constrained DNKOs, or statite-like support conditions are first identified here. Instead, the contribution is threefold: to propose a continuous Dyson support spectrum as a more useful architecture language than a purely discrete taxonomy; to identify MDDS as a low-$\beta$ segment of that spectrum; and to reorganize known low-latitude displaced-orbit dynamics into a compact analytic criterion for layered Dyson-swarm design. In that sense, the paper is best read as an analytic architecture/framework paper: it imports mature astrodynamical ingredients, translates them into a Dyson-swarm design language, and shows that the resulting low-angle regime is non-empty and physically interpretable.

Figure 1 should be read as the motivating geometric picture for the paper, while Figures 2-5 are intended to track the transition from architecture concept to quantitative support curve and then to low-latitude examples.

![Figure 1: Keplerian deadlock geometry](../figures/concept/fig1_keplerian_deadlock.svg)
...
Figure 2 is intended to visualize this displaced geometry at the architectural level, while Figure 3 captures the resulting support curve in compact form.

![Figure 2: MDDS stratified rings](../figures/concept/fig2_mdds_stratified_rings.svg)
...
![Figure 3: Support curves](../figures/results/fig3_support_curves.svg)
*Figure 3. Latitude support curves. (a) Full range showing $\beta_{\min}(\phi)$ with critical thresholds: the $\beta = 1$ architecture threshold at $\phi \approx 22.6°$ (where bubble/statite designs become viable) and the payload-optimized branch endpoint at $\phi_c \approx 35.3°$, $\beta = 1.5$ (where orbital contribution vanishes along the present branch, not across the full continuum). (b) Low-latitude detail showing both $\beta_{\min}(\phi)$ and $\sigma_{\max}(\phi)$ with reference values at 0.1°, 0.5°, and 1°.*
...
Figure 4 should be read as the main-text illustrative window for the low-angle regime, anchored by the explicit representative points listed above.

![Figure 4: Low-latitude feasibility window](../figures/results/fig4_low_latitude_window.svg)
*Figure 4. Low-latitude feasibility window showing maximum supportable areal density $\sigma_{\max}$ at representative latitudes. Even the entry-level $0.1^\circ$ case already produces substantial geometric separation while remaining far more permissive in areal-density terms than the better-known high-latitude or bubble limits.*
...
Figure 5 should therefore be read not as a second feasibility boundary, but as an operational-geometry correction layered on top of the same support framework.

![Figure 5: Earth-synchronous radius correction](../figures/results/fig5_sync_radius_shift.svg)
*Figure 5. Earth-synchronous radius correction along the payload-optimized branch. The inward shift from 1 AU grows with latitude but does not alter the fundamental support curve—synchronization is an operational geometry constraint, not an additional support penalty.*
...
The main result of this paper is best understood as a two-level reframing. At the architectural level, MDDS is not presented here as a completed Dyson-engineering solution, but as a progressive alternative to the conventional Keplerian Dyson swarm, one that replaces intersecting swarm geometry with radiation-assisted latitude bands and thereby motivates a lower orbital-topology burden. More broadly, the paper argues that Dyson architectures are more coherently understood as a continuous support-and-stratification spectrum than as a purely discrete shell/swarm/bubble taxonomy. At the physical level, its significance is that it identifies a usefully screenable low-angle regime in which modest radiation-pressure support can produce large off-plane stratification without crossing into the payload-hostile bubble limit.
...
The paper also suggests a useful deployment interpretation. Since the support curve is most permissive near the ecliptic, a Dyson-progressive architecture can begin in low-latitude bands and expand outward in latitude as areal density improves. On this view, MDDS is not merely a static configuration concept; it is also a growth logic for stratified stellar infrastructure. The low-angle examples sharpen that point: in the Sun-Earth environment, even very small angular displacements already produce wide deployment planes, and the corresponding areal-density thresholds range from permissive to entry-level rather than immediately futuristic. At the same time, the present paper does not yet quantify swarm-management benefit using a formal collision or conjunction metric; that benefit should therefore be read here as an architectural rationale, not as a fully closed traffic-analysis result.

That said, the topology claim can already be stated more sharply than a purely qualitative intuition. In a same-radius Keplerian swarm, non-coplanar orbital planes necessarily intersect at nodal lines, so the number of geometric crossing corridors grows rapidly with swarm richness. In the MDDS framing, by contrast, low-latitude displaced bands are separated primarily along the normal direction while preserving essentially the same heliocentric collection radius. The immediate architectural effect is therefore not merely ``more space,'' but a change in the geometry of interactions: from repeated line-of-node crossings to layered bands with explicit normal separation. A natural next-step metric family would include node-intersection count, minimum normal separation, and conjunction-corridor density. The present paper stops one layer short of that formal traffic analysis, but it defines the geometry in a way that makes such metrics well posed.
...
The present manuscript stops short of the heavier engineering questions. Detailed structural closure, fixed bus mass, deployment mechanics, perturbation-control closure, and broader power-system comparisons remain important, but they are follow-on problems. The appropriate claim at this stage is that the framework is mathematically coherent, physically interpretable, and non-empty in the low-latitude regime.

## 5. Conclusion

We have introduced a low-$\beta$ framework for Micro-Displaced Dyson Swarm architectures and derived a closed-form support relation linking off-plane latitude to required radiation-pressure support. This yields a simple density-based feasibility criterion and exposes two natural design directions: a payload-optimized branch and a synchronization-constrained branch. The contribution is threefold: at the architectural level, the paper advances a continuous Dyson support-spectrum view in which swarm-like and bubble-like configurations occupy different regions of a common support space; within that spectrum, MDDS emerges as a progressive low-$\beta$ alternative to the conventional Keplerian Dyson swarm by recasting known displaced-orbit dynamics into a layered Dyson-swarm design language; and at the physical level, the Sun-Earth environment already contains a non-empty low-angle operating window in which modest angular displacement produces large spatial separation while entry-level characteristic angles approach, and in some cases enter, the range of present lightweight spacecraft systems. The main contribution is therefore not a new solar-sail orbit family or a full engineering realization, but a compact and usable framework for analyzing stratified Dyson-swarm architectures.
